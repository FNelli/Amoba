...
void Palya::event_loop()
{
    ...
    while(gin >> ev && ev.keycode != key_escape) {
        ...
        if (!start && elsokor) {
            ...
            if (focus!=-1) {
                _menu._startmenu[focus]->_checked = 1;
                _menu._startmenu[focus]->handle(ev);
            }
            for (size_t i=0; i<_menu._startmenu.size(); i++) {
                if (i != focus) {
                    _menu._startmenu[i]->_checked = 0;
                }
                _menu._startmenu[i]->draw(tikktakk);
            }
            if (_menu._startmenu[1]->_checked) {
            ...
            if (!keslelteto && ev.type == ev_mouse && ev.button==btn_left && ev.button > 0 && nyertes.size()<2) {
                for (int i=0; i<(int)_y/_meret; i++) {
                    for (int e=0; e<(int)_x/_meret; e++) {
                        if (_palya[e][i]->is_selected(ev.pos_x, ev.pos_y) && _palya[e][i]->_checked == 0 && nyertes.size()<2) {
                            lepesek++;
                            focus_x = e;
                            focus_y = i;
                            _palya[e][i]->_checked = lepesek%2+1;
        ...
        /// MENU - ELSO KOR UTAN
        if (!start && !elsokor) { // EZ AZERT VAN BENNE, HOGY EGY PICIT LEHESSEN LATNI A NYERTES ABRASOROZATOT
            if (ev.type == 3  && varj<2) {
                varj++;
            }
            if (varj > 1) {
                ...
                if (focus!=-1) {
                    _menu._vegemenu[focus]->_checked = 1;
                    _menu._vegemenu[focus]->handle(ev);
                }
                for (size_t i=0; i<_menu._vegemenu.size(); i++) {
                    if (i != focus) {
                        _menu._vegemenu[i]->_checked = 0;
                    }
                    _menu._vegemenu[i]->draw(tikktakk);
                }
                _menu._k->draw(0);
                if (_menu._vegemenu[2]->_checked) {
                    start = false;
                    elsokor = true;
                    focus = -1;
                    _jatekos1pont = 0;
                ...
                if (_menu._vegemenu[1]->_checked) {
                    start = true;
                    elsokor = false;
                    focus = -1;
                }
                /// PALYA URITES
                if (_menu._vegemenu[1]->_checked || _menu._vegemenu[2]->_checked) {
        ...
        if (lepesek == _palya.size()*_palya[0].size()-1) { // KEZELI HA BETELIK A PALYA
                start = false;
                elsokor = false;
                focus = -1;
                _menu._k->_elsop = "DONTETLEN...";
                _menu._k->_masodikp = "DOBBENETES";
        }
        gout << refresh;
    }
}
