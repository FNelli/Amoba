...
vector<int> jatekvege(vector<vector<int>> jatekallas, int ex, int iy, int jatekos)
{
    ...
    // FEL
    int fel_le = iy-1;
    while (!nincstobb  && fel_le>=0 && !bingo) {
    ...
    fel_le = iy+1;
    nincstobb = false;
     while (!nincstobb && fel_le<ylimit && !bingo) {
        if (jatekallas[ex][fel_le] == jatekos) {
            nyertesabrak.push_back(ex);
            nyertesabrak.push_back(fel_le);
            db++;
    ...
    nincstobb = false;
    int jobb_bal = ex+1;
    while (!nincstobb && jobb_bal<xlimit && !bingo) {
    ...
    while (!nincstobb && jobb_bal>=0) {
        if (jatekallas[jobb_bal][iy] == jatekos && !bingo) {
            nyertesabrak.push_back(jobb_bal);
            nyertesabrak.push_back(iy);
            db++;
            jobb_bal--;
    ...
    fel_le = iy-1;
    jobb_bal = ex-1;
    while (!nincstobb && fel_le>=0 && jobb_bal>=0 && !bingo) {
    ...
    fel_le = iy+1;
    jobb_bal = ex+1;
    while (!nincstobb && fel_le<ylimit && jobb_bal<xlimit && !bingo) {
        if (jatekallas[jobb_bal][fel_le] == jatekos) {
            nyertesabrak.push_back(jobb_bal);
            nyertesabrak.push_back(fel_le);
            db++;
    ...
    fel_le = iy+1;
    jobb_bal = ex-1;
    while (!nincstobb && fel_le<ylimit && jobb_bal>=0 && !bingo) {
        if (jatekallas[jobb_bal][fel_le] == jatekos) {
            nyertesabrak.push_back(jobb_bal);
            nyertesabrak.push_back(fel_le);
            db++;
    ...
    fel_le = iy-1;
    jobb_bal = ex+1;
    while (!nincstobb  && fel_le>=0 && jobb_bal<xlimit && !bingo) {
        if (jatekallas[jobb_bal][fel_le] == jatekos) {
            nyertesabrak.push_back(jobb_bal);
            nyertesabrak.push_back(fel_le);
            db++;
